...
if seccion == "Predicción":
    st.markdown(
        "<h1 style='text-align:center; font-size:32px; font-weight:bold;'>⚡ Predicción de Demanda Eléctrica ⚡</h1>",
    ...
    # Función para slider con bloque coloreado
    ...
    demanda_lag_168 = demanda_slider_coloreada("Demanda hace 7 días")
    media_movil_24h = demanda_slider_coloreada("Media últimas 24 horas")
    ...
    with col1:
        ...
        st.markdown(
            f"<div style='margin:5px 0; font-weight:bold; font-size:16px; color:#f39f18;'>Hora seleccionada: {hora_real}h {icono}</div>",
            unsafe_allow_html=True
        )
    ...
    st.markdown(f"<div style='margin:5px 0; font-weight:bold; font-size:16px;'>Día seleccionado: {dia_nombre}</div>", unsafe_allow_html=True)
    st.markdown(f"<div style='margin:5px 0; font-weight:bold; font-size:16px; color:#f39f18;'>Es fin de semana: {es_finde_texto}</div>", unsafe_allow_html=True)
    ...
    if mes in [12,1,2]:
        ...
        estacion = "🍂 Otoño"
    st.markdown(f"<div style='margin:5px 0; font-weight:bold; font-size:16px;'>{estacion}</div>", unsafe_allow_html=True)

    # ---------------------------------
    # TEMPERATURA POR REGIÓN
    # ---------------------------------
    st.subheader("🌡️ Temperatura por Región")
...
# ===========================
    ...
    # Texto de ejemplo + imágenes
    st.header("1. El Problema de Negocio y el Contexto")
    st.write("Decidimos abordar uno de los problemas más costosos y críticos del sector industrial: la predicción de la demanda eléctrica.")
    st.write("El sistema eléctrico no puede almacenar energía a gran escala; lo que se genera debe consumirse al instante.")
    st.write("El reto principal que enfrentamos no fue solo técnico, sino de comportamiento: la demanda eléctrica es el resultado de millones de decisiones humanas.")
    st.image(BASE_DIR / "data" / "Images" / "01.png", use_column_width=True)

    st.header("2. La Estrategia de Datos")
    st.write("Al principio, planteamos la hipótesis de que la demanda dependía casi exclusivamente de la temperatura. Sin embargo, al analizar los datos en profundidad, nos dimos cuenta de que las variables climáticas clásicas solo tenían una correlación moderada con el consumo real (alrededor de un 0.4).")
    ...
    st.write("Llegamos a la conclusión de que el mejor predictor del consumo actual no es el clima, sino el pasado inmediato:")
    st.write("1. Cuánto consumimos hace una hora.")
    st.write("2. Cuánto consumimos ayer a esta misma hora.")
    st.write("3. Cuánto consumimos la semana pasada.")
    st.write("Por ello, construimos variables de 'Lags' o retardos temporales.")
    st.image(BASE_DIR / "data" / "Images" / "03.png", use_column_width=True)

    st.header("4. La Batalla de Modelos: XGBoost vs N-BEATS")
    st.write("Con los datos listos, llegamos a la fase de modelado. Decidimos poner a competir a nuestro modelo basado en árboles (XGBoost) contra una arquitectura de Deep Learning moderna (N-BEATS).")
    st.write("En las métricas, N-BEATS nos dio un R^2 negativo (-34). Fue incapaz de encontrar patrones estables con el volumen de datos disponible.")
    st.write("En contraste, nuestro modelo XGBoost alcanzó el 0.99.")
    st.image(BASE_DIR / "data" / "Images" / "04.png", use_column_width=True)

    st.header("5. Validación y Resultados")
    st.write("Para evitar engañarnos con métricas de entrenamiento, diseñamos una validación temporal estricta.")
    st.write("Mantuvimos el R^2 superior a 0.99 y, visualmente, el modelo replicó perfectamente la dinámica diaria y las caídas de los fines de semana.")
    st.image(BASE_DIR / "data" / "Images" / "05.png", use_column_width=True)

    st.header("6. Limitaciones y Observaciones")
    st.write("Dependencia del Dato Reciente: Nuestro modelo depende mucho del dato de 'hace una hora'.")
    st.write("Eventos Imprevisibles: Si ocurre una nueva pandemia o una crisis energética anómala, el modelo tardará en reaccionar.")
    st.write("Falta de Variables Económicas: Actualmente miramos calendario y clima. Pero sabemos que el precio de la luz afecta al consumo industrial.")
    st.write("Filomena fue un outlier.")
